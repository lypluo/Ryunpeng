convertROI<-function (path_img_ref, path_ROIs, file.type = ".jpg") 
{
  ...
  for (a in 1:nrois) {
    ...
    tmp.array <- array(dim = c(nrow(out.mask), ncol(out.mask), 
                               1))
    ...
    the.pol <- Polygon(as.matrix(coordinates(x)[[1]][[1]]), 
                       hole = FALSE)
    the.pol.final <- SpatialPolygons(list(Polygons(list(the.pol), 
                                                   ID = 1)))
